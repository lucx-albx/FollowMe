...
import { faLocationArrow, faHouse, faQrcode } from '@fortawesome/free-solid-svg-icons'

const Footer = () => {
    const EmIcon = <FontAwesomeIcon icon={faHouse} className='pointer'/>
    ...
    useEffect(() => {
        ...
        if (location.pathname === '/QrCode') {
            ...
            sezione('em');
        } else if (location.pathname === '/Sicurezza' || location.pathname === '/Sicurezza/Percorso') {
            sezione('si');
        } else {
            sezione('rest');
        }
    ...
    return (
        <div className='MainFooter' ref={footerRef}>
            <div className='FooterIcon'>
                ...
                <NavLink to={"/"} onClick={()=>sezione('em')} style={{ textDecoration: 'none', color: '#319547' }} className={icone.em[1] === "ns" ? 'SezioneImpostataH' : 'iconaStyleH'}>
                    {icone.em[0]}
                </NavLink>
