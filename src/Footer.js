...
import { faHouse, faQrcode, faUser } from '@fortawesome/free-solid-svg-icons'

const Footer = () => {
    const EmIcon = <FontAwesomeIcon icon={faUser} className='pointer'/>
    const QrIcon = <FontAwesomeIcon icon={faQrcode} className='pointer'/>
    const SiIcon = <FontAwesomeIcon icon={faHouse} className='pointer'/>
    ...
    return (
        <div className='MainFooter' ref={footerRef}>
            <div className='FooterIcon'>
                ...
                <NavLink to={"/"} onClick={()=>sezione('si')} style={{ textDecoration: 'none', color: '#319547' }} className={icone.si[1] === "ns" ? 'SezioneImpostataH' : 'iconaStyleH'}>
                    {icone.si[0]}
                </NavLink>

                <NavLink to={"/About"} onClick={()=>sezione('em')} style={{ textDecoration: 'none', color: '#319547' }} className={icone.em[1] === "ns" ? 'SezioneImpostata' : 'iconaStyle'}>
                    {icone.em[0]}
                </NavLink>
